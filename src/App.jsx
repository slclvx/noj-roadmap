...
const C = {
  ...
  border:"rgba(140,150,240,0.15)",
  borderM:"rgba(140,150,240,0.28)",
  text:"#1A1D3A",   sub:"#4A4E80",    mute:"#9A9EC8",
  // Dark
  dBg:"#0C0F1E",    dCard:"#141828",  dSoft:"#1A1F35",
  dBorder:"rgba(140,160,255,0.1)",
  dBorderM:"rgba(140,160,255,0.22)",
  dText:"#EEEEFF",  dSub:"#8890C8",   dMute:"#353A60",
};

const AVATAR_BG = [C.blue,C.purple,C.pink,C.teal,C.orange,C.green,C.red];
...
function NewBadge() {
  return <span style={{fontFamily:"'Nunito',sans-serif",fontSize:9,fontWeight:900,padding:"2px 9px",borderRadius:99,background:"linear-gradient(135deg,#FFB800,#FF7828)",color:"#fff",boxShadow:"0 2px 6px rgba(255,120,0,0.4)"}}>✦ NEW</span>;
}

function StatCard({value,label,color,dark}) {
  return (
    <div style={{padding:"14px 22px",borderRadius:22,textAlign:"center",background:dark?`${color}18`:`${color}14`,border:`2.5px solid ${color}50`,boxShadow:`0 4px 20px ${color}25`,minWidth:90}}>
  ...
  return (
    <div style={{marginTop:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        ...
        <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:color}}>{pct}%</span>
      </div>
      <div style={{height:6,borderRadius:99,background:`${color}18`,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${pct}%`,borderRadius:99,background:`linear-gradient(90deg,${color},${color}cc)`,transition:"width 0.4s ease",boxShadow:`0 0 8px ${color}66`}}/>
      </div>
    </div>
  );
}
...
const LS_PHASE_CHECKS = "noj_phase_checks";

function RoadmapPage({dark}) {
  const t=T(dark);
  const [open,setOpen]=useState(1);
  const [tab,setTab]=useState("milestones");
  const [phaseStatus,setPhaseStatus]=useState(()=>LS.get(LS_PHASE_STATUS,{}));
  const [checks,setChecks]=useState(()=>LS.get(LS_PHASE_CHECKS,{}));

  useEffect(()=>LS.set(LS_PHASE_STATUS,phaseStatus),[phaseStatus]);
  useEffect(()=>LS.set(LS_PHASE_CHECKS,checks),[checks]);
  ...
  const cycleStatus=(e,id)=>{
    e.stopPropagation();
    setPhaseStatus(p=>{
      const cur=p[id]||"Not Started";
      const idx=QUEST_STATUSES.indexOf(cur);
      return{...p,[id]:QUEST_STATUSES[(idx+1)%QUEST_STATUSES.length]};
    });
  };

  const toggleCheck=(phaseId,milestoneIdx)=>{
          ...
          return (
            <div key={phase.id} id={`p${phase.id}`} style={{marginBottom:10}}>
              <button onClick={()=>toggle(phase.id)} style={{width:"100%",textAlign:"left",cursor:"pointer",outline:"none",background:isOpen?(dark?`${phase.accent}14`:`${phase.accent}09`):t.card,border:`2.5px solid ${isOpen?phase.accent+"66":t.border}`,borderRadius:isOpen?"22px 22px 0 0":"22px",padding:"16px 20px",display:"flex",alignItems:"center",gap:14,transition:"all 0.2s ease",boxShadow:isOpen?`0 8px 32px ${phase.accent}22`:t.shadowS}}>
                ...
                {/* Status badge */}
                <div onClick={e=>cycleStatus(e,phase.id)} style={{fontFamily:"'Nunito',sans-serif",fontSize:10,fontWeight:800,flexShrink:0,padding:"4px 11px",borderRadius:99,background:sm.bg,color:sm.color,border:`1.5px solid ${sm.color}44`,cursor:"pointer",transition:"all 0.2s",whiteSpace:"nowrap",userSelect:"none"}} title="Click to change status">⟳ {status}</div>
                  ...
                  {tab==="milestones"&&(
                    <div>
                      {phase.milestones.map((m,i)=>{
                        ...
                        return (
                          <div key={i} onClick={e=>{e.stopPropagation();toggleCheck(phase.id,i);}} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"9px 0",borderBottom:`1px solid ${t.border}`,cursor:"pointer",opacity:done?0.55:1,transition:"opacity 0.2s"}}>
                            <Check done={done} accent={phase.accent} onClick={e=>{e.stopPropagation();toggleCheck(phase.id,i);}}/>
                      ...
                      <div style={{marginTop:12}}>
                        <ProgressBar value={phaseChecks} total={phase.milestones.length} color={phase.accent}/>
                      </div>
...
// ─── RESOURCES PAGE ────────────────────────────────────────────────────────────
const LS_BOOKS = "noj_books";

function ResourcesPage({dark}) {
  const t=T(dark);
  const [openPhase,setOpenPhase]=useState(null);
  const [filter,setFilter]=useState("All");
  const [read,setRead]=useState(()=>LS.get(LS_BOOKS,{}));

  useEffect(()=>LS.set(LS_BOOKS,read),[read]);

  const toggleRead=(num,e)=>{e.stopPropagation();setRead(p=>({...p,[num]:!p[num]}));};
  ...
  return (
    <div>
      <div style={{padding:"52px 20px 44px",maxWidth:900,margin:"0 auto",textAlign:"center",borderBottom:`2px solid ${t.border}`}}>
        ...
        <h2 style={{fontFamily:"'Fredoka One',cursive",fontSize:"clamp(26px,5vw,52px)",marginBottom:12,lineHeight:1.1,color:C.orange}}>My Reading Roadmap</h2>
              ...
              {isOpen&&(
                <div style={{background:dark?`${phase.accent}05`:`${phase.accent}03`,border:`2.5px solid ${phase.accent}44`,borderTop:"none",borderRadius:"0 0 22px 22px",padding:"4px 20px 20px"}}>
                  <div style={{padding:"10px 0 4px"}}>
                    <ProgressBar value={phaseRead} total={phase.books.length} color={phase.accent}/>
                  </div>
                  {phase.books.map((book,i)=>{
                    ...
                    return (
                      <div key={book.num} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"14px 0",borderBottom:i<phase.books.length-1?`1.5px solid ${t.border}`:"none",opacity:done?0.5:1,transition:"opacity 0.2s"}}>
                        <Check done={done} accent={phase.accent} onClick={e=>toggleRead(book.num,e)}/>
                        <div style={{fontFamily:"'Fredoka One',cursive",fontSize:13,color:phase.accent,minWidth:30,padding:"3px 7px",borderRadius:9,textAlign:"center",background:dark?`${phase.accent}16`:`${phase.accent}10`,border:`2px solid ${phase.accent}44`,flexShrink:0}}>{String(book.num).padStart(2,"00")}</div>
                        <div style={{flex:1}}>
...
// ─── SIDE QUESTS PAGE ──────────────────────────────────────────────────────────
const LS_QUESTS = "noj_quests";
const LS_QUEST_CHECKS = "noj_quest_checks";
...
function SideQuestsPage({dark}) {
  ...
  const [quests,setQuests]=useState(()=>{
    const saved=LS.get(LS_QUESTS,null);
    if(saved)return DEFAULT_QUESTS.map(q=>({...q,status:saved[q.id]||q.status}));
    return DEFAULT_QUESTS;
  });
  const [questChecks,setQuestChecks]=useState(()=>LS.get(LS_QUEST_CHECKS,{}));

  useEffect(()=>{
    const m={};quests.forEach(q=>{m[q.id]=q.status;});
    LS.set(LS_QUESTS,m);
  },[quests]);
  useEffect(()=>LS.set(LS_QUEST_CHECKS,questChecks),[questChecks]);
  ...
  const toggleCheck=(questId,milestoneIdx)=>{
    const key=`${questId}-${milestoneIdx}`;
    setQuestChecks(p=>({...p,[key]:!p[key]}));
  };
  ...
  return (
    <div>
      <div style={{padding:"52px 20px 44px",maxWidth:900,margin:"0 auto",textAlign:"center",borderBottom:`2px solid ${t.border}`}}>
        ...
        <h2 style={{fontFamily:"'Fredoka One',cursive",fontSize:"clamp(26px,5vw,52px)",marginBottom:12,lineHeight:1.1,color:C.red}}>Life Beyond the Main Story</h2>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap",marginBottom:24}}>
          ...
          <StatCard value={`${doneCount}/${totalMilestones}`} label="Milestones" color={C.purple} dark={dark}/>
        </div>
        <div style={{maxWidth:400,margin:"0 auto"}}>
          <ProgressBar value={doneCount} total={totalMilestones} color={C.red}/>
        </div>
        <div style={{marginTop:14,fontFamily:"'Nunito',sans-serif",fontSize:12,color:t.mute,fontWeight:700}}>✏️ Click a status badge to update it · Click milestones to check them off</div>
      </div>
      ...
      <div style={{maxWidth:900,margin:"12px auto 0",padding:"0 20px 64px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:16}}>
        {filtered.map(quest=>{
            ...
            <Card key={quest.id} dark={dark} accent={quest.color} onClick={()=>setOpen(isOpen?null:quest.id)}>
              ...
              <div style={{padding:"18px 20px 20px"}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:12}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"'Fredoka One',cursive",fontSize:17,color:T(dark).text,marginBottom:6}}>{quest.title}</div>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                      <span onClick={e=>cycleStatus(e,quest.id)} style={{fontFamily:"'Nunito',sans-serif",fontSize:10,fontWeight:800,padding:"3px 10px",borderRadius:99,background:sm.bg,color:sm.color,border:`1.5px solid ${sm.color}44`,cursor:"pointer",userSelect:"none"}}>⟳ {quest.status}</span>
                      <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:T(dark).mute,letterSpacing:1}}>{quest.category}</span>
                    </div>
                  </div>
                </div>

                <p style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:T(dark).sub,lineHeight:1.65,fontWeight:600,margin:"0 0 10px"}}>{quest.description}</p>

                {/* Mini progress */}
                <ProgressBar value={qDone} total={quest.milestones.length} color={quest.color}/>

                {isOpen&&(
                  <div style={{marginTop:16,paddingTop:16,borderTop:`1.5px solid ${T(dark).border}`}}>
                    ...
                    {quest.startDate&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,color:T(dark).mute,fontWeight:700,marginBottom:12}}>Started: {quest.startDate}</div>}
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:T(dark).mute,letterSpacing:2,textTransform:"uppercase",marginBottom:8,fontWeight:700}}>Milestones</div>
                    {quest.milestones.map((m,i)=>{
                      const done=!!questChecks[`${quest.id}-${i}`];
                      return (
                        <div key={i} onClick={e=>{e.stopPropagation();toggleCheck(quest.id,i);}} style={{display:"flex",gap:8,alignItems:"flex-start",padding:"6px 0",cursor:"pointer",opacity:done?0.5:1,transition:"opacity 0.2s"}}>
                          <Check done={done} accent={quest.color} size={20} onClick={e=>{e.stopPropagation();toggleCheck(quest.id,i);}}/>
                          <div style={{fontFamily:"'Nunito',sans-serif",fontSize:12,color:T(dark).sub,lineHeight:1.6,fontWeight:600,textDecoration:done?"line-through":"none",paddingTop:1}}>{m}</div>
                        </div>
                      );
                    })}
                  </div>
...
// ─── ROOT APP ──────────────────────────────────────────────────────────────────
const BG_DOTS=Array.from({length:14},(_,i)=>({
